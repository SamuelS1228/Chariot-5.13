    ...
    with st.sidebar:
        ...
        with st.expander('📍 Locations', expanded=False):
            ...
            for idx in range(1, 11):
                ...
                en = col1.checkbox(f'{idx}', key=f'fix_en_{name}_{idx}',
                                   value=scn.get(f'fix{idx}_en', False))
                scn[f'fix{idx}_en'] = en
                if en:
                    with col2:
                        lon = st.number_input('Longitude', format='%.6f',
                                              value=float(scn.get(f'fix{idx}_lon', 0.0)),
                                              key=f'fix_lon_{name}_{idx}')
                        lat = st.number_input('Latitude', format='%.6f',
                                              value=float(scn.get(f'fix{idx}_lat', 0.0)),
                                              key=f'fix_lat_{name}_{idx}')
                        scn[f'fix{idx}_lon'] = lon
                        scn[f'fix{idx}_lat'] = lat
